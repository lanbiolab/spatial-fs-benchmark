...
class GraphST():
    def __init__(self, 
        ...
        if isinstance(graph_neigh, (csc_matrix, csr_matrix)):
           graph_neigh = graph_neigh + eye(self.adj.shape[0], dtype=np.float32, format='csr')
           self.graph_neigh = sparse_mx_to_torch_sparse_tensor(graph_neigh).coalesce().to(self.device)
        else:
           self.graph_neigh = torch.FloatTensor(graph_neigh + np.eye(self.adj.shape[0])).to(self.device)

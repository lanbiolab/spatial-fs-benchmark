...
heatmap_panel <- function(metric, title, legend_title) {
    ...
    ggplot(frame, aes(x = .data$Reference, y = .data$Method)) +
        ...
        geom_text(aes(label = sprintf("%.2f", .data$Mean)), size = 1.7, colour = "#17202A") +
        scale_fill_gradientn(colours = c("#F1F5F9", "#7DD3FC", "#0369A1"), limits = c(0, 1), name = legend_title) +
...
rank_frame <- held_summary |>
    ...
    ungroup() |>
    select(.data$Method, .data$Reference, .data$Metric, .data$Rank) |>
    pivot_wider(names_from = .data$Reference, values_from = .data$Rank)
...
panel_c <- ggplot(rank_frame, aes(x = .data[["Moran's I"]], y = .data$nnSVG)) +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed", linewidth = 0.35, colour = "#94A3B8") +
    geom_point(shape = 21, fill = "#0EA5A4", colour = "white", stroke = 0.3, size = 1.8) +
    ...
    labs(title = "c  Rankings agree across held-out statistics", x = "Rank using Moran's I reference", y = "Rank using nnSVG reference") +
    base_theme
...
panel_d <- ggplot(association, aes(x = .data$SpearmanRho, y = .data$Measure, colour = .data$Reference)) +
    geom_vline(xintercept = 0, linewidth = 0.3, colour = "#CBD5E1") +
    geom_errorbarh(aes(xmin = .data$Lower95, xmax = .data$Upper95), height = 0.16, linewidth = 0.55, position = position_dodge(width = 0.38)) +
    geom_point(size = 2, position = position_dodge(width = 0.38)) +
    scale_colour_manual(values = c("Moran's I" = "#D97706", "nnSVG" = "#0284C7"), name = "Held-out reference") +
    scale_x_continuous(limits = c(-0.25, 0.72), breaks = c(-0.2, 0, 0.2, 0.4, 0.6)) +
    labs(title = "d  Reproducibility only weakly tracks ARI", x = "Fold-bootstrap Spearman correlation with ARI", y = NULL) +
    base_theme +
    theme(legend.position = "bottom")

panel_a <- heatmap_panel("MeanReferencePercentile", "a  Held-out reference percentile", "Mean\npercentile")
panel_b <- heatmap_panel("Jaccard", "b  Held-out top-N overlap", "Mean\nJaccard")
figure_s7 <- (panel_a | panel_b) / (panel_c | panel_d) +
    plot_layout(heights = c(1.28, 0.82), guides = "collect") &
...
hybrid_palette <- c(
    "Balanced HVG-SVG union" = "#0284C7",
    "Exact HVG-SVG intersection" = "#D97706",
    "Scanpy Seurat v3, batch" = "#7C3AED",
    "Moran's I" = "#0F766E",
    "triku" = "#475569",
    "SOMDE" = "#64748B",
    "Seurat VST" = "#94A3B8"
...
panel_a8 <- ggplot(hybrid_effective, aes(x = .data$effective_n_features, y = reorder(.data$Dataset, .data$effective_n_features), colour = .data$Method)) +
    geom_vline(xintercept = 2000, linetype = "dashed", linewidth = 0.35, colour = "#94A3B8") +
    geom_point(size = 2.2) +
    scale_colour_manual(
    ...
    labs(title = "a  The exact intersection is a compact, variable-size set", x = "Effective number of selected genes", y = NULL) +
    base_theme +
    theme(legend.position = "bottom")
...
panel_b8 <- ggplot(hybrid_comparison, aes(x = .data$CoreOverallMean, y = .data$Method, colour = .data$Method)) +
    ...
    labs(title = "b  Hybrid constructions differ downstream", x = "CoreOverall score", y = NULL) +
    base_theme
...
panel_c8 <- ggplot(hybrid_diff, aes(x = .data$DifferenceFromBestParent, y = reorder(.data$Dataset, .data$DifferenceFromBestParent), colour = .data$Method)) +
    geom_vline(xintercept = 0, linewidth = 0.35, colour = "#64748B") +
    ...
    labs(title = "c  Difference from the better parent selector", x = "CoreOverall difference from best parent", y = NULL) +
    base_theme +
...
panel_d8 <- ggplot(hybrid_global, aes(x = .data$GlobalRankWithHybrids, y = .data$Method, colour = .data$Method)) +
    geom_segment(aes(x = 1, xend = .data$GlobalRankWithHybrids, yend = .data$Method), colour = "#CBD5E1", linewidth = 0.45) +
    geom_point(size = 2) +
    facet_wrap(~Integrator, nrow = 1) +
    scale_colour_manual(values = hybrid_palette, guide = "none") +
    scale_x_continuous(limits = c(1, 31), breaks = c(1, 5, 10, 20, 30)) +
    labs(title = "d  Global rank after adding the two hybrid controls", x = "Global rank (1 = best)", y = NULL) +
    base_theme

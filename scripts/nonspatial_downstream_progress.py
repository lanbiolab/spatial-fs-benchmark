...
CONFIG_ROOT = ROOT / "configs" / "rebuild_v1" / "non_spatial_downstream"
INTEGRATOR_VERSIONS = {
...
def main() -> None:
    ...
    for dataset_name in sorted(expected):
        ...
        print(f"{dataset_name:28s} {bar(done, count)} {done:4d}/{count:<4d} {pct:6.1f}%")
    pct = 100 * total_done / total_expected if total_expected else 100
    print(f"{'TOTAL':28s} {bar(total_done, total_expected)} {total_done:4d}/{total_expected:<4d} {pct:6.1f}%")
